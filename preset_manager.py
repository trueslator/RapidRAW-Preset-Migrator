...
def _source_to_fallback_rel(source: str) -> str:
    ...
    filename = safe_name(pathlib.Path(pp.name).stem) + ("__xmp" if pathlib.Path(pp.name).suffix.lower() == ".xmp" else "") + ".rrpreset"
...
def generate_catalog(output_dir: str | pathlib.Path) -> Dict[str, Any]:
    ...
    with report.open(encoding="utf-8-sig", newline="") as f:
        ...
        for r in reader:
            ...
            rows.append({
                ...
                "source": source,
                "format": pathlib.PurePosixPath(source).suffix.lower().lstrip("."),
    ...
    catalog = {
        ...
        "stats": {
            ...
            "formats": dict(__import__("collections").Counter(x["format"] for x in rows)),
        },
    }
...
<div class="notice"><b>Hinweis:</b> Die Migration ist eine Annäherung, keine 1:1-Reproduktion von Lightroom. Unterschiedliche RAW-Engines und Verarbeitungspipelines können sichtbar abweichende Ergebnisse erzeugen. Näherungen und nicht übertragbare Einstellungen stehen im Konvertierungsbericht.</div>
<div id="status" class="status"></div>
<div class="controls"><input id="q" type="search" placeholder="Preset, Ordner oder Profil suchen…"><select id="group"><option value="">Alle Ordner</option></select><select id="type"><option value="">Farbe + B&W</option><option value="color">Farbe</option><option value="bw">B&W</option><option value="dcp">CameraProfile verfügbar</option><option value="missing">CameraProfile fehlt</option><option value="favorite">Nur Favoriten</option><option value="selected">Nur ausgewählt</option><option value="xmp">Nur XMP</option><option value="lrtemplate">Nur lrtemplate</option></select><button id="refresh">Aktualisieren</button></div>
...
function visible(p){let q=$('#q').value.trim().toLowerCase(),g=$('#group').value,t=$('#type').value;if(g&&p.group!==g)return false;let ps=matches(p).map(id=>profileById(id)).filter(Boolean);let hay=`${p.name} ${p.group} ${p.format||''} ${p.cameraProfile} ${ps.map(x=>x.camera+' '+x.name).join(' ')}`.toLowerCase();if(q&&!hay.includes(q))return false;let s=selection.get(p.id);if(t==='bw'&&!p.bw)return false;if(t==='color'&&p.bw)return false;if(t==='dcp'&&!ps.length)return false;if(t==='missing'&&(!p.cameraProfile||ps.length))return false;if(t==='favorite'&&!favorites.has(p.id))return false;if(t==='selected'&&!s?.on)return false;if(t==='xmp'&&p.format!=='xmp')return false;if(t==='lrtemplate'&&p.format!=='lrtemplate')return false;return true}
